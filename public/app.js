...
function getExchangeTitle(exchangeId) {
    const labels = {
        binance: 'Binance',
        kraken: 'Kraken',
        bybit: 'Bybit',
...
function getMarketMakingLoopDescription(keepListening) {
    return keepListening
        ? 'loop contínuo ativo pelo .env'
        : 'loop até encontrar oportunidade favorável e encerrar';
}

function formatMarketMakingMode(mode) {
...
function initializeExchangePanels() {
    ...
    for (const exchangeId of visibleExchanges) {
        ...
        const view = {
            ...
            mmGadget: root.querySelector('[data-role="mm-gadget"]'),
            mmSummary: root.querySelector('[data-role="mm-summary"]'),
            mmTarget: root.querySelector('[data-role="mm-target"]'),
            mmLastRun: root.querySelector('[data-role="mm-last-run"]'),
            mmModeLoop: root.querySelector('[data-role="mm-mode-loop"]'),
            mmActiveStatus: root.querySelector('[data-role="mm-active-status"]'),
            mmMetrics: root.querySelector('[data-role="mm-metrics"]'),
...
function infoMetricCard(label, value) {
...
function logCard(item) {
    ...
    const body = entries.length
        ? entries.map((entry) => `<div class="row"><span>${entry.route}</span><strong class="positive">${formatNumber(entry.percentage)}%</strong></div>`).join('')
